...
export function AtsResumePreview({ resume }: AtsResumePreviewProps) {
  ...
  const experienceEntries = getFilledExperience(resume)
          ...
          <Section title="Experience">
            <div className="space-y-4">
              {experienceEntries.map((entry, index) => (
                <div key={`${entry.company}-${index}`} className="space-y-1">
                  <p className="text-base font-semibold text-slate-900">
                    {[entry.role, entry.company].filter((value) => isNonEmpty(value)).join(", ")}
                  </p>
                  {[entry.duration, entry.location].some((value) => isNonEmpty(value)) && (
                    <p className="text-sm text-slate-700">
                      {[entry.duration, entry.location].filter((value) => isNonEmpty(value)).join(" | ")}
                    </p>
                  )}
                  {isNonEmpty(entry.description) && (
                    <p className="whitespace-pre-line text-sm text-slate-700">{entry.description.trim()}</p>
                  )}
                </div>
              ))}
            </div>
